...
"""MLX implementation of Qwen3.8-Flash-Next (``model_type: "qwen4_exp"``).

**Vendored, not original.** This is ml-explore/mlx-lm PR #1788 ("Add
Qwen3.8-Flash-Next (qwen4_exp) model support", branch ``eauchs:add-qwen4-exp``,
MIT) carried here unmodified apart from rewriting its four relative imports
absolute, because the PR is still open and no mlx-lm release ships ``qwen4_exp``.
``compat.py`` aliases this module into ``sys.modules`` as
``mlx_lm.models.qwen4_exp`` and **self-disables the moment mlx-lm ships the type
natively**, so when #1788 lands this file stops being consulted with no code
change. Delete it at that point.
...
* **Qwen Sparse Attention (QSA)** -- ``QSAIndexer`` mean-pools keys into blocks
  of ``indexer_compress_ratio`` (4) and top-k selects ``indexer_budget // 4`` =
  512 blocks per query. Below ``indexer_budget`` (2048) tokens it returns
  ``None`` and ordinary causal masking applies. **The indexer's
  ``index_qk_proj`` is kept at full precision** by
  ``quantize_model._is_router``: it makes a discrete choice of which blocks to
  read, so quantization error changes *which* KV is attended rather than
  degrading the result smoothly.
* **Gated residual / hyper-connections** -- ``GatedResidual`` replaces the plain
  residual add around both the attention and MLP blocks.
* **Sharded n-gram / PLE embedding** -- a 20M-row n-gram table (51.2B params,
  28% of the model) plus a per-layer embedding injected at ``ple_layer_ids``.
* **MoE shape** -- 512 routed experts at top-10 with a narrow
...
class Attention(nn.Module):
    ...
    def __call__(self, x, rope, mask, cache, idx_cache, rope_cs=None) -> mx.array:
        B, S, _ = x.shape
        ...
        # `rope_cs` is a precomputed (cos, sin) for THIS step's token positions,
        # used by the vision path to supply 3-D MRoPE positions for image tokens
        # (see models/qwen4_exp_vision.py). Text-only callers pass nothing and get
        # the 1-D positions below, unchanged. The QSA indexer above keeps using
        # `rope` for its block positions either way -- blocks are sequence
        # positions, not image coordinates.
        cos, sin = rope_cs if rope_cs is not None else rope(_positions(offset, S))
...
class DecoderLayer(nn.Module):
    ...
    def __call__(self, h, rope, mask, conv_mask, cache, idx_cache, ids, prev_ctx,
        ...
        if self.layer_type == "linear_attention":
            ...
            x = self.self_attn(x, rope, mask, cache, idx_cache, rope_cs)
...
class Qwen4ExpModel(nn.Module):
    ...
    def __call__(self, ids: mx.array, cache=None, input_embeddings=None, rope_cs=None):
        ...
        for layer, c in zip(self.layers, cache):
            ...
            h = layer(h, self.rope, mask, conv_mask, c, idx_c, ids, prev_ctx, rope_cs)
...
class Model(nn.Module):
    ...
    def __call__(self, inputs: mx.array, cache=None, input_embeddings=None, rope_cs=None):
        out = self.model(inputs, cache, input_embeddings, rope_cs)
